...
class PackAll extends React.Component {
    constructor(props) {
        super(props);
        ...
        axios.post('https://3k-store-api.gigagames.co.th/products',{userId: d},
    {
        ...
        this.getCookieTab("tab");

        setInterval(()=>{
            if(this.state.tabIndex == 1){
                document.getElementById("all").style.opacity = "1.0"
                document.getElementById("hero").style.opacity = "0.4"
                document.getElementById("upgrade").style.opacity = "0.4"
                document.getElementById("item").style.opacity = "0.4"
            } else if(this.state.tabIndex == 2){
                document.getElementById("all").style.opacity = "0.4"
                document.getElementById("hero").style.opacity = "1.0"
                document.getElementById("upgrade").style.opacity = "0.4"
                document.getElementById("item").style.opacity = "0.4"
            } else if(this.state.tabIndex == 3){
                document.getElementById("all").style.opacity = "0.4"
                document.getElementById("hero").style.opacity = "0.4"
                document.getElementById("upgrade").style.opacity = "1.0"
                document.getElementById("item").style.opacity = "0.4"
    ...
    getCookieTab(cname) {
        ...
        for(var i = 0; i < ca.length; i++) {
          ...
          console.log(c.substring(name.length, c.length));
            if(c.substring(name.length, c.length)=="2"){
                console.log('Logged in')
                this.setState({
                    tabIndex: 2
                })
            } else if(c.substring(name.length, c.length)=="3"){
                console.log('Logged in')
                this.setState({
                    tabIndex: 3
                })
            } else if(c.substring(name.length, c.length)=="4"){
                console.log('Logged in')
                this.setState({
                    tabIndex: 4
                })
            } else if(c.substring(name.length, c.length)=="1"){
                console.log('Logged in')
                this.setState({
                    tabIndex: 1
                })
    ...
    heroTab(){
        ...
        var z = document.getElementsByClassName("hero").length;
        var i;
        var j;
        var k;
        for(k=0;k<z;k++){document.getElementById("contRow").getElementsByClassName("hero")[k].style.display = "block";}
        for(i=0;i<x;i++){document.getElementById("contRow").getElementsByClassName("item")[i].style.display = "none";}
        for(j=0;j<y;j++){document.getElementById("contRow").getElementsByClassName("equipment")[j].style.display = "none";}
        // document.getElementById("tkd.i7.web").style.display="none";
    }
    upgradeTab(){
        var x = document.getElementsByClassName("hero").length;
        var y = document.getElementsByClassName("item").length;
        var z = document.getElementsByClassName("equipment").length;
        var i;
        var j;
        var k;
        for(k=0;k<z;k++){document.getElementById("contRow").getElementsByClassName("equipment")[k].style.display = "block";}
        for(i=0;i<x;i++){document.getElementById("contRow").getElementsByClassName("hero")[i].style.display = "none";}
        for(j=0;j<y;j++){document.getElementById("contRow").getElementsByClassName("item")[j].style.display = "none";}
        // document.getElementById("tkd.i7.web").style.display="none";
    }
...
<Col xs={3} sm={3} md={3} style={{height: 48, cursor: 'pointer'}} id={"Allbutt"+index} onClick={()=>{sendOrd(packId, channId)}}><img style={{position: 'absolute', left: 0, width: '100%', height: 54}} src={require('./asset/Home/BUY Button.png')}></img><div style={{zIndex: 666, position: 'absolute', textAlign: "center", alignContent: "center", width: '100%'}}><h1 id="hotPrice0" id={"Allprice"+index} style={{margin: "7px 0px"}}>{value.amount} บาท</h1></div></Col>
            ...
            if(ab.length > 3){
                ...
                // document.getElementById(abc).style.border = "thick solid white";
                // document.getElementById(abc).style.borderWidth = "30px"
            }
            });
                ...
                <SectionContainer>
                    <SectionContainerExtension>
                        ...
                        <Row style={{backgroundImage: `url(${BGTab})`}}>
                            <Col xs={3} sm={3} md={3}><div id="all" onClick={() => this.setState({ tabIndex: 1 })} style={{cursor: 'pointer'}}><h1 id="allText" style={{position: 'absolute'}}>ALL( {this.state.pak.length} )</h1><img style={{width: '90%'}} src={require('./asset/pack/packall/BUtton [ 0 ] EVENT[No-text].png')}></img></div></Col>
                            <Col xs={3} sm={3} md={3}><div id="hero" onClick={() => this.setState({ tabIndex: 2 })} style={{cursor: 'pointer'}}><img style={{width: '90%'}} src={require('./asset/pack/packall/buttHero.png')}></img></div></Col>
                            <Col xs={3} sm={3} md={3}><div id="upgrade" onClick={() => this.setState({ tabIndex: 3 })} style={{cursor: 'pointer'}}><img style={{width: '90%'}} src={require('./asset/Home/Button Equipment.png')}></img></div></Col>
                            <Col xs={3} sm={3} md={3}><div id="item" onClick={() => this.setState({ tabIndex: 4 })} style={{cursor: 'pointer'}}><img style={{width: '90%'}} src={require('./asset/pack/packall/buttItem.png')}></img></div></Col>
                        </Row>
